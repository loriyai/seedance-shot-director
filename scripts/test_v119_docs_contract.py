...
class V119DocumentationContractTests(unittest.TestCase):
    ...
    def test_fast_path_and_hard_rules_remain_explicit(self):
        ...
        self.assertIn('末镜只在存在真实约束时输出一次', core)
        self.assertIn('块时长按自然内容在 4–15 秒之间取整', core)
        self.assertIn('每块至少 1 个移动镜头', core)
        ...
        self.assertIn('查表得出', skill)
        self.assertIn('自检表', skill)
