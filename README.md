...
本版保持完整15秒默认五镜，可按表演与连续运镜需要调整，优先完整收尾，必要时最后两块以实际时长联合重排。2.0的30秒目标编译为不超过15秒的块，2.5可使用原生30秒；实际入口限制另行记录。
...
V1.14 用一份结构化规划生成提示词和台账，编译内置一次硬检查，随后一次语义复核；通过即交付，有实际问题才局部修复。审阅稿局部修改默认只展示变化并提供完整文件，原版与历史版本仍完整保存。长剧本首次建立全局索引，处理到对应场次再补详细执行档案。
...
直投正文隐藏制作ID、收尾模式和语速预算；具名自然语言台词保留必要语气，每镜附分号分隔的环境音效。人物外观以用户素材为准；跨场景首尾各留至少1秒动态无口播画面，后台同时校验整句和逐镜片段容量。风格选择完整展示固定三项，不附解释。
