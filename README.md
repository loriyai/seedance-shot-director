...
V1.28 修两条成片质量问题：一是长话轮被"单镜 5 秒上限"逼着在不该切的地方硬切——现在明确块内长话轮保留完整话轮并用 `span` 拆镜、片段只用于跨块，切点优先级为情绪或意图转折 > 语义停顿 > 5 秒上限 > 纯标点，并加机械检查：同一块内出现同一话轮的多片段即警告（实测在 seg001 上报出 03、04、08、10–14 块共 8 处）。二是 `reaction` 字段被塞进静态位置细节、渲染成"强制描写……"这类生硬命令——现在 `reaction` 只写本镜真正发生的动态反馈，静态位置与持续状态写进 `environment`／`blocking`，四个渲染模板的"强制"字样改为使得／随之／可见／导致。副反应的"动态 vs 静态"机械判定在实测中误报 17 条（多为正常动作反应），按"先量再上闸门"原则未采纳，只保留规则与措辞修正。新增 `scripts/test_v128_split_gates.py` 回归切点规则。
