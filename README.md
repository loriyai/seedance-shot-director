...
- 默认画幅为 `16:9`。
- 完整剧本先进入输入登记与配置阶段：依次确认目标时长、视频风格和是否输出资产列表；配置完成后等待用户发送片段，再开始生成。
- 完整剧本超过 `150000` 个 Unicode 字符时触发疑似误发确认门禁，用户确认无误前暂停后续处理。
...
- 完整剧本输入登记、15 万字疑似误发确认、时长/风格/资产列表三项配置和等待片段门禁。
