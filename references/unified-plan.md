...
- `reaction`：来源支持或低风险物理副反应，用于“引发……”“导致……”分句；只写本镜真正发生的动态反馈，静态位置与持续状态写进 `environment` 或 `blocking`。
...
- 块内长话轮一律保留完整 `text` 并用 `span` 拆镜；只有话轮真的跨生成块时才登记片段（跨块片段不能用 `span`）。切点优先级：情绪或意图转折 > 语义停顿 > 单镜 5 秒上限 > 纯标点；不得为迁就 5 秒上限把情绪转折两侧压进同一镜。
