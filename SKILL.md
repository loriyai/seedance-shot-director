...
- 所有模式共用来源边界：执行细化只补已成立动作的路径、接触、可见状态和摄影安排；不得在分镜阶段二次增加表演节拍。仅提及道具不证明它在场或由某人持有。身份、目标、说话人、关键道具、因果和时空若存在影响执行的歧义，才补问。
...
2. 完整剧本做首次人物、场景、道具、时间线和空间建档；后续来源修订增量更新。收集缺失的时长、风格、资产列表选择，随后等待片段或明确处理范围，不自动生成开头。片段先保存 V0。详见 [intake-gate.md](references/intake-gate.md)。
...
4. 默认执行简版、轻度增强强度、无配乐、独立收束；支持本任务增强偏好、导演详版、连续剪辑和声音配置。风格采用 [style-profiles.md](references/style-profiles.md) 的已有选项或自定义；项目风格不写入技能安装目录。无论是否展示资产列表，都建立 [asset-list-spec.md](references/asset-list-spec.md) 所需档案。
5. 用户选择增强时按 [script-enhancement.md](references/script-enhancement.md) 输出完整标记审阅稿，等待确认。要求改善台词、冲突或结构时先给单独的具体编剧建议；未经选择的建议不入稿。直接分镜或使用原版则锁定当前基线。
...
3. 按 [generation-block-splitting.md](references/generation-block-splitting.md) 自然分块并优先完整收尾。完整 15 秒块严格五镜；不足四秒尾文先尝试合并有余量的前块，必要时联合调整最后两块实际时长，不能默认延期或虚构动作填秒。模型最小时长无法容纳整个超短输入且无自然余韵时，保留全文并说明具体待决事项。30 秒与实际短块规则见该文件。
4. 按 [shot-splitting.md](references/shot-splitting.md)、[timing-allocation.md](references/timing-allocation.md) 确定视觉边界。一个编号是一镜；来源事件只发生一次，但其过程和话轮可以跨镜分段呈现。完成 [generation-block-ledger.md](references/generation-block-ledger.md) 的映射与入口/出口。
5. 用 [generation-block-contract.md](references/generation-block-contract.md) 和 [prompt-template.md](references/prompt-template.md) 编译中文提示词。稳定事实放块头，必要起点放首镜；逐镜写“画面与动作”“摄影机与构图”，台词与必要声音、衔接按需出现。声音时间用共同格式显式绑定话轮，原文正文仅出现一次。
...
- 按 [dialogue-coverage.md](references/dialogue-coverage.md)、[qc-fallback.md](references/qc-fallback.md) 检查；典型问题参考 [failure-repair-playbook.md](references/failure-repair-playbook.md)。运行 `scripts/validate_dialogue_and_timeline.py --source <锁定来源.txt> --output <提示词.txt> --duration <实际编译上限> --model <2.0或2.5>`；入口不确定时不假填型号，使用已核实的能力限制。
- 声音起止、台词覆盖镜头、换人顺序和静音尾部必须一致。发音估计不确定时警告复核；确定的改词或错归属不能被其他未知段落降级。
- 15 秒完整块严格五镜；30 秒建议 8–12 镜，偏离需有导演依据；末尾短块不以凑镜数或秒数制造新剧情。
- 独立收束默认最后 0.3–0.8 秒无对白；连续剪辑的非终结块可保留持续动作或声音衔接，剧情明确硬切时服从来源。具体收尾配置按模型与制作配置文档，不把每块都处理成剧情结束。
- 空镜只服务时间、空间、留白或转场；15 秒通常至多一镜且不超过 1.5 秒，不连续用空镜凑时长。不靠重复握拳、凝视、推镜、皱眉或无意义环境动态填满五镜。
- 执行版保护顺序：来源与声音身份 → 连续性 → 表演与摄影执行 → 必要光线声音 → 风格装饰。详版使用同一份规划，审阅台账置于可复制块之外。分层见 [prompt-layering.md](references/prompt-layering.md)、[visual-style-system.md](references/visual-style-system.md)。
